...
def query_gemini(prompt_text, timeout_seconds=60):
    ...
    if re.search(career_keywords, prompt_text, re.IGNORECASE) or "career" in prompt_text.lower() or "job" in prompt_text.lower():
        ...
        try:
            herkey_snippet = fetch_herkey_info(prompt_text)
            if herkey_snippet:
                prompt_with_herkey = f"{prompt_text}\n\nAlso, HerKey has some relevantinfo:\n{herkey_snippet}"
            else:
                prompt_with_herkey = prompt_text
            contents = [{"parts": [{"text": prompt_with_herkey}]}]
            response = model.generate_content(contents) 
            if response.text:
