...
def query_gemini(prompt_text, timeout_seconds=60):
    ...
    if re.search(career_keywords, prompt_text, re.IGNORECASE):
        logging.info(f"[{timestamp}] Career-related query detected. Sending refined response.")
        try:
            contents = [{"parts": [{"text": prompt_text}]}]
            response = model.generate_content(contents)
            if response.text:
                relevant_text = response.text[:2000]  # Limit the response to 2000 chars
                # Trim non-relevant or excessive details
                relevant_text = ' '.join(relevant_text.split()[:250])  # Limit to ~250 words
                logging.info(f"[{timestamp}] Career-related response (first 50 chars): {relevant_text[:50]}...")
